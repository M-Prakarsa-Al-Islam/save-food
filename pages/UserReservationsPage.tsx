...
const ReservationCard: React.FC<{reservation: Reservation}> = ({ reservation }) => {
    ...
    const isExpired = new Date(foodItem.expiresAt) < new Date();
    const isCompleted = reservation.status === 'completed';
    const isActive = reservation.status === 'active' && !isExpired;
...
const UserReservationsPage: React.FC<UserReservationsPageProps> = ({ onSelectFoodItem, onProfileClick }) => {
    ...
    const reservations = useMemo(() => {
        ...
        return [];
    }, [dataContext, authContext?.currentUser]);
    
    const activeReservations = reservations.filter(r => r.status === 'active' && new Date(dataContext?.getFoodItemById(r.foodItemId)?.expiresAt || 0) > new Date());
    const pastReservations = reservations.filter(r => r.status !== 'active' || new Date(dataContext?.getFoodItemById(r.foodItemId)?.expiresAt || 0) <= new Date());
