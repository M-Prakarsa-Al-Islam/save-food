...
const ReservationDetailCard: React.FC<{ reservation: Reservation }> = ({ reservation }) => {
    ...
    return (
        <div className={`p-4 rounded-lg shadow-sm border ${isExpired ? 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800 opacity-70' : 'bg-white dark:bg-slate-800 dark:border-slate-700'}`}>
            <div className="flex justify-between items-start">
                ...
                {isExpired && (
                    <div className="text-xs font-semibold px-2 py-1 bg-red-100 text-red-800 rounded-full dark:bg-red-900 dark:text-red-300">{t('expired')}</div>
                )}
            </div>
