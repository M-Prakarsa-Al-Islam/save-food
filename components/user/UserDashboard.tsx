...
const UserDashboard: React.FC<UserDashboardProps> = ({ onSelectFoodItem, onProfileClick }) => {
  ...
  const { expiringSoon, filteredFoodItems } = useMemo(() => {
    let items = foodItems.filter(item => item.stock > 0 && new Date(item.expiresAt) > new Date());
    
    const twoHoursFromNow = new Date(Date.now() + 2 * 60 * 60 * 1000);
    const expiring = items.filter(item => new Date(item.expiresAt) < twoHoursFromNow).sort((a,b) => new Date(a.expiresAt).getTime() - new Date(b.expiresAt).getTime());
    ...
    return { expiringSoon: expiring, filteredFoodItems: items };
  ...
  return (
    <>
      ...
      <div className="p-4 space-y-6">
        <div>
            <h1 className="text-2xl font-bold text-dark dark:text-light">{t('welcome', { name: userName })}</h1>
        ...
        {expiringSoon.length > 0 && (
          <section>
            <h2 className="text-xl font-bold text-dark dark:text-light mb-3 flex items-center gap-2"><Flame className="text-secondary"/> {t('dontMissOut')}</h2>
