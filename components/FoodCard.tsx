  ...
  return (
    ...
    >
      <div className="relative aspect-square">
        <img src={foodItem.imageUrl} alt={foodItem.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
        {isFree && (
            <div className="absolute top-2 left-2 bg-secondary text-white text-xs font-bold px-2 py-1 rounded-full flex items-center gap-1">
                <Tag size={14}/>
                {t('freeTag')}
            </div>
        )}
         <button onClick={handleLikeClick} className="absolute top-2 right-2 bg-white/70 p-1.5 rounded-full text-red-500 hover:bg-white hover:text-red-600 focus:outline-none transition-all duration-300" aria-label="Add to favorites">
      ...
      <div className="p-4 flex flex-col flex-grow">
        <div className="flex-grow">
          <div className="flex items-center gap-1.5">
            <p className="text-sm text-gray-500 dark:text-gray-400 truncate">{restaurant.name}</p>
        ...
        <div className="mt-4 flex justify-between items-end">
          ...
          <div className="text-right">
            <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">
              {t('stock')}: <span className="text-primary font-bold text-base">{foodItem.stock}</span>
            </p>
