...
const initialFoodItems: FoodItem[] = [
    { id: 'food-1', restaurantId: 'resto-1', name: 'Croissant Coklat', description: 'Croissant renyah dengan isian coklat premium yang meleleh di mulut.', imageUrl: 'https://images.unsplash.com/photo-1555507036-ab1f4038808a?q=80&w=870&auto=format&fit=crop', stock: 5, originalPrice: 20000, discountedPrice: 10000, expiresAt: new Date(Date.now() + 2 * 60 * 60 * 1000), isSurpriseMeal: false, category: 'Roti', pickupAddress: 'Jl. Roti Enak No. 1', pickupLat: -6.21, pickupLng: 106.81 },
    { id: 'food-3', restaurantId: 'resto-3', name: 'Surprise Pastry Box', description: 'Kotak kejutan berisi aneka macam kue dan roti pilihan dari chef kami hari ini. Isinya berbeda setiap hari!', imageUrl: 'https://images.unsplash.com/photo-1563729784474-d77dbb933a9e?q=80&w=774&auto=format&fit=crop', stock: 10, originalPrice: 50000, discountedPrice: 0, expiresAt: new Date(Date.now() + 4 * 60 * 60 * 1000), isSurpriseMeal: true, category: 'Kue', pickupAddress: 'Jl. Kafein No. 10', pickupLat: -6.20, pickupLng: 106.83 },
    { id: 'food-4', restaurantId: 'resto-1', name: 'Roti Gandum', description: 'Roti gandum utuh yang sehat dan kaya serat, cocok untuk sarapan atau camilan sehat.', imageUrl: 'https://images.unsplash.com/photo-1509440159596-0249088772ff?q=80&w=872&auto=format&fit=crop', stock: 2, originalPrice: 25000, discountedPrice: 0, expiresAt: new Date(Date.now() + 0.5 * 60 * 60 * 1000), isSurpriseMeal: false, category: 'Roti', pickupAddress: 'Jl. Roti Enak No. 1', pickupLat: -6.21, pickupLng: 106.81 },
];
...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    // Simulate real-time expiration
    const interval = setInterval(() => {
      const now = new Date();
      setFoodItems(prev => prev.filter(item => new Date(item.expiresAt) > now));
      // No longer auto-cancel reservations here, let them show as expired.
  ...
  const createReservation = (foodItemId: string): Reservation => {
    ...
    if (!foodItem || foodItem.stock <= 0) {
     ...
     if (new Date(foodItem.expiresAt) < new Date()) {
      throw new Error('error.foodExpired');
    }
  ...
  const addFoodItem = (formData: any) => {
    ...
    const newFoodItem: FoodItem = {
        ...
        expiresAt: new Date(Date.now() + parseFloat(formData.expiresInHours) * 60 * 60 * 1000),
